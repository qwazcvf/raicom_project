...
class PulseWalk(Node):
    ...
    def wait_really_stopped(self, timeout=8.0, v_threshold=0.05, drift_threshold=0.05, stable_time=0.8, min_z=0.50):
        """
        v2 停稳判断：
        只看 odom 实际速度 vxy 和 z 高度。
        不再用过严的 drift 判断，避免明明已经停了却被误判超时。
        """
        print("等待真正停止...")
        stable_start = None
        t0 = time.time()
        ...
        while time.time() - t0 < timeout:
            ...
            v = self.vxy()
            stopped_now = (v < v_threshold) and (p.z > min_z)
            ...
            last_pose = (p.x, p.y, p.z, v)

            if time.time() - last_print > 0.5:
                print(
                    f"\r停止检测: x={p.x:.3f}, y={p.y:.3f}, z={p.z:.3f}, "
                    f"vxy={v:.3f}, stopped={stopped_now}",
                    end="",
                    flush=True,
                )
        ...
        if last_pose is not None:
            x, y, z, v = last_pose
            print(f"末状态: x={x:.3f}, y={y:.3f}, z={z:.3f}, vxy={v:.3f}")
            if v < 0.08 and z > min_z:
                print("末状态速度很小，按已停稳处理")
                return True
